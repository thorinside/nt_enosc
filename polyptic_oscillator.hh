...
public:
  ...
  {
    f r =
        numOsc > 1_f ? amplitude : numOsc > 0_f ? amplitude * numOsc
                                                : 0_f;
    amplitudes += r;
    amplitude *= balance;
    numOsc--;
...
public:
               ...
               Buffer<f, block_size> &out1, Buffer<f, block_size> &out2)
  {
    ...
    f atten = 0.5_f / sum;
    atten *= 1_f + Data::data->normalization_factors.interpolate_from_index(sum.max(f(kMaxNumOsc)));
...
class Oscillators : Nocopy
{
  ...
  {
    return mode == ALTERNATE ? !(i & 1) : mode == LOW_HIGH ? i < numOsc / 2
                                                           : i == 0;
  }
  ...
  public:
    FrequencyAccumulator(Scale const &g, f r, f p, f s, f d) : root(r), pitch(p), spread(s), detune(d), scale(g) {}
...
public:
               ...
               Buffer<f, block_size> &out1, Buffer<f, block_size> &out2)
  {
    ...
    atten *= 0.5_f + (0.5_f + Data::data->normalization_factors[numOsc]) * balance;
...
public:
  PolypticOscillator(Parameters &params) : params_(params)
  {
    ...
    previous_scale_mode = params_.scale.mode;
  }

  // Expose quantizer scale access and reset for JSON serialization
  Scale *get_scale(Parameters::Scale scaleParam) { return quantizer_.get_scale(scaleParam); }
  void reset_scale(Parameters::Scale scaleParam) { quantizer_.reset_scale(scaleParam); }
  // Immediately select the specified scale so it is used in the next Process()
  void select_current_scale() { current_scale_ = quantizer_.get_scale(params_.scale); }
  ...
  void Process(Buffer<Frame, block_size> &out)
  {
    ...
    {
      if (previous_scale_index != params_.scale.value || previous_scale_mode != params_.scale.mode)
      {
