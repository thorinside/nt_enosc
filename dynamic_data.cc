...
/* triangles */
static constexpr Buffer<Buffer<s8_0, 9>, 8> triangles_12ths = {{{
...
DynamicData::DynamicData() {
  ...
  { //60 bytes of code for triangle tables
    for (int i = 0; i < 8; i++)
      for (int j = 0; j < 9; j++)
        triangles[i][j] = (f)(triangles_12ths[i][j]) / 12.0_f;
  }
  // Init normalization factors to unity
  for (int i = 0; i <= kMaxNumOsc; ++i)
    normalization_factors[i] = 1_f;
}
